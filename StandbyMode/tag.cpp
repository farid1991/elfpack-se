...
int id3_tag_attachframe(struct id3_tag *tag, struct id3_frame *frame)
{
  ...
  frames = (struct id3_frame **)realloc(tag->frames, (tag->nframes + 1) * sizeof(*frames));
  if (frames == 0)
    return -1;
		 ...
		 char *text, unsigned long number)
{
  ...
  if (text)
    id3_latin1_decode((unsigned char*)text, ucs4);
  else
