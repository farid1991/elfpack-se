...
void _merge_from_v2_11(v2_12_config_t *newcfg, v2_11_config_t *oldcfg)
{
//Картинки ...
//Картинка 1
...
//Время 2...
  ...
  newcfg->cfg_clock2_mediapleyer_show = 1;
...
//Емкость батареи...
  ...
  newcfg->cfg_batt_images_custom = 0;
...
//Индикация заряда...
  newcfg->cfg_batt_charge_anim = 1;
  newcfg->cfg_batt_charge_show = oldcfg->cfg_batt_charge_show;
  newcfg->cfg_batt_charge_custom = 0;
...
int merge_from_v2_11(v2_12_config_t *newcfg, void *oldcfg, int oldcfgsize)
{
  int result = -1;
  if(oldcfgsize == sizeof(v2_11_config_t))
  {
    _merge_from_v2_11(newcfg, (v2_11_config_t *)oldcfg);
    result = 0;
  }
  else
  {
    char *buf;
    if ((buf=new char[sizeof(v2_11_config_t)]))
    {
      if(merge_from_v2_10((v2_11_config_t *)buf, oldcfg, oldcfgsize) == 0)
      {
        _merge_from_v2_11(newcfg, (v2_11_config_t *)buf);
        result = 0;
      }
    }
    delete buf;
