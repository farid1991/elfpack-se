...
void _merge_from_v2_17(v2_18_config_t *newcfg, v2_17_config_t *oldcfg)
{
//Картинки ...
//Картинка 1
...
//Двоичные часы...
  newcfg->cfg_clock3_show = 0;
  newcfg->cfg_clock3_x = 0;
  newcfg->cfg_clock3_y = 0;
  wstrcpy(newcfg->cfg_clock3_resource, L"%INI_INT%/Standby Mode/binclock/default.png");
//Дата...
...
//День...
  ...
  newcfg->cfg_alarm_icon_show = oldcfg->cfg_alarm_icon_show;
...
//Емкость батареи...
  ...
  newcfg->cfg_batt_bar_reverse = oldcfg->cfg_batt_bar_reverse;
...
//Индикация светодиодами
  newcfg->cfg_charge_Led_show=0;
  newcfg->cfg_charge_Led_OnTime = 1000;
  newcfg->cfg_charge_Led_OffTime = 3000;
//Индикация красным светодиодом...
  newcfg->cfg_charge_RedLed_show = 0;
  newcfg->cfg_charge_RedLed_OnTime = 1000;
  newcfg->cfg_charge_RedLed_OffTime = 3000;
//Голосовая индикация...
  newcfg->cfg_charge_say_levels_up=0;
  newcfg->cfg_charge_say_levels_down=0;
  newcfg->cfg_charge_begin_from=20;
//Настройки звука
  wstrcpy(newcfg->cfg_charge_folder_path, L"%INI_INT%/Standby Mode/batt/Sound");
  newcfg->cfg_charge_volume=3;
  newcfg->cfg_charge_silent = 0;
//Настройки времени работы
  newcfg->cfg_charge_TimeFrom.hour = 8;
  newcfg->cfg_charge_TimeFrom.min  = 0;
  newcfg->cfg_charge_TimeFrom.sec  = 0;
  newcfg->cfg_charge_TimeTo.hour = 22;
  newcfg->cfg_charge_TimeTo.min  = 0;
  newcfg->cfg_charge_TimeTo.sec  = 0;
//Температура батареи...
...
//Время/Дата/День/Будильник
  ...
  newcfg->cfg_clock3_not_show_if_player = 0;
...
//Длительность звонка
  newcfg->cfg_call_not_show_if_player = oldcfg->cfg_call_not_show_if_player;
//Приложения...
...
//Иконки статуса...
  ...
  newcfg->cfg_status_msg_show = oldcfg->cfg_status_msg_show;
  newcfg->cfg_status_download_show = oldcfg->cfg_status_download_show;
  newcfg->cfg_status_phonenum_state_show = 0;
...
#define STATUS_ICN_CFG_D_NEW(num, file) \
  ...
  STATUS_ICN_CFG_D_NEW(34, L"by_net_num.png");
  STATUS_ICN_CFG_D_NEW(35, L"hide_num.png");
  STATUS_ICN_CFG_D_NEW(36, L"show_num.png");
...
//Длительность звонка
  newcfg->cfg_call_show = oldcfg->cfg_call_show;
  newcfg->cfg_call_font = oldcfg->cfg_call_font;
  newcfg->cfg_call_highlight = oldcfg->cfg_call_highlight;
  newcfg->cfg_call_color = oldcfg->cfg_call_color;
  newcfg->cfg_call_border = oldcfg->cfg_call_border;
  newcfg->cfg_call_align = oldcfg->cfg_call_align;
  newcfg->cfg_call_x = oldcfg->cfg_call_x;
  newcfg->cfg_call_y = oldcfg->cfg_call_y;
  newcfg->cfg_call_icon_show = oldcfg->cfg_call_icon_show;
  wstrcpy(newcfg->cfg_call_file, oldcfg->cfg_call_file);
...
int merge_from_v2_17(v2_18_config_t *newcfg, void *oldcfg, int oldcfgsize)
{
  int result = -1;
  if(oldcfgsize == sizeof(v2_17_config_t))
  {
    _merge_from_v2_17(newcfg, (v2_17_config_t *)oldcfg);
    result = 0;
  }
  else
  {
    char *buf;
    if ((buf=new char[sizeof(v2_17_config_t)]))
    {
      if(merge_from_v2_16((v2_17_config_t *)buf, oldcfg, oldcfgsize) == 0)
      {
        _merge_from_v2_17(newcfg, (v2_17_config_t *)buf);
        result = 0;
      }
    }
    delete buf;
