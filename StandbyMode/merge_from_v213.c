...
void _merge_from_v2_13(v2_14_config_t *newcfg, v2_13_config_t *oldcfg)
{
//Картинки ...
//Картинка 1
...
//День...
  ...
  //Будильник...
  newcfg->cfg_alarm_show = 0;
  newcfg->cfg_alarm_font = 0;
  newcfg->cfg_alarm_color = 0xFFFFFFFF;
  newcfg->cfg_alarm_border = 0xFF000000;
  newcfg->cfg_alarm_align = 0;
  newcfg->cfg_alarm_x = 0;
  newcfg->cfg_alarm_y = 0;
  wstrcpy(newcfg->cfg_alarm_file, L"%INI_INT%/Standby Mode/alarm.png");
  newcfg->cfg_refresh_alarm = 2000;
...
//Время/Дата/День/Будильник
  ...
  newcfg->cfg_alarm_not_show_if_player = 0;
//Heap/Батарея/Температура
...
//Иконки статуса...
  ...
  newcfg->cfg_status_alarm_show = 1;
...
#define STATUS_ICN_CFG_D_NEW(num, file) \
  ...
  STATUS_ICN_CFG_D_NEW(23, L"alarm.png");
...
int merge_from_v2_13(v2_14_config_t *newcfg, void *oldcfg, int oldcfgsize)
{
  int result = -1;
  if(oldcfgsize == sizeof(v2_13_config_t))
  {
    _merge_from_v2_13(newcfg, (v2_13_config_t *)oldcfg);
    result = 0;
  }
  else
  {
    char *buf;
    if ((buf=new char[sizeof(v2_13_config_t)]))
    {
      if(merge_from_v2_12((v2_13_config_t *)buf, oldcfg, oldcfgsize) == 0)
      {
        _merge_from_v2_13(newcfg, (v2_13_config_t *)buf);
        result = 0;
      }
    }
    delete buf;
