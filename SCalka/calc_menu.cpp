...
void InsertVar_CreateMenu(MyBOOK *mbk)
{
	GUI_LIST *lo;
	TEXTID sid;
	mbk->calc_insvar=lo=CreateListMenu(mbk,0);
...
void AngleSelect_CreateCBoxGui(MyBOOK *mbk)
{
	...
	TEXTID strid[3];
	GUI_ONEOFMANY *om;
	mbk->angle_sel_list=om=CreateOneOfMany(mbk);
...
void AutocalcSelect_CreateCBoxGui(MyBOOK *mbk)
{
	...
	TEXTID strid[2];
	GUI_ONEOFMANY *om;
	mbk->autocalc_sel_list=om=CreateOneOfMany(mbk);
...
void CreateCalcSettings(MyBOOK *mbk)
{
	GUI_LIST *lo;
	TEXTID sid;
	mbk->calc_settings=lo=CreateListMenu(mbk,0);
	sid=TextID_Create(L"Settings",ENC_UCS2,TEXTID_ANY_LEN);
	GUIObject_SetTitleText(lo,sid);
...
void CreateCalcMenu(void)
{
	...
	{
		MyBOOK *mbk=(MyBOOK *)bk;
		mbk->calc_menu=lo=CreateListMenu(mbk,0);
		sid=TextID_Create(L"Menu",ENC_UCS2,TEXTID_ANY_LEN);
		int ids=0;
		texts[ids++]=TextID_Create(L"Set formula",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Insert var",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Add formula",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"View vars",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Ans To X",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Ans To Y",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Settings",ENC_UCS2,TEXTID_ANY_LEN);
		texts[ids++]=TextID_Create(L"Exit",ENC_UCS2,TEXTID_ANY_LEN);
		OneOfMany_SetTexts((GUI_ONEOFMANY *)lo,texts,ids);
		GUIObject_SetTitleText(lo,sid);
		ListMenu_SetItemCount(lo,ids);
		ListMenu_SetCursorToItem(lo,0);
